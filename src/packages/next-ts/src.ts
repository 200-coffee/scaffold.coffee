...
const createSrc = async (cwd: string): Promise<void> => {
  // create the src directory
  await promises.mkdir(`${cwd}/src`).catch((err) => {
    throw err;
  });
};

const createApp = async (cwd: string): Promise<void> => {
  // create the app directory
  await promises.mkdir(`${cwd}/src/app`).catch((err) => {
    throw err;
  });

  const layoutTSX = `import clsx from "clsx";
...
`;
  ...
  const pageTSX = `import { Branding } from "@/ui/branding";
...
`;
  ...
  await promises
    .writeFile(`${cwd}/src/app/layout.tsx`, layoutTSX)
    .catch((err) => {
      throw err;
    });
  await promises.writeFile(`${cwd}/src/app/page.tsx`, pageTSX).catch((err) => {
    throw err;
  });
};

const createComponents = async (cwd: string): Promise<void> => {
  // create the components directory
  await promises.mkdir(`${cwd}/src/components`).catch((err) => {
    throw err;
  });

  const themeTSX = `"use client";
...
`;
  ...
  await promises
    .writeFile(`${cwd}/src/components/theme.tsx`, themeTSX)
    .catch((err) => {
      throw err;
    });
};

const createHooks = async (cwd: string): Promise<void> => {
  // create the hooks directory
  await promises.mkdir(`${cwd}/src/hooks`).catch((err) => {
    throw err;
  });
};

const createLayouts = async (cwd: string): Promise<void> => {
  // create the layouts directory
  await promises.mkdir(`${cwd}/src/layouts`).catch((err) => {
    throw err;
  });
};

const createServer = async (cwd: string): Promise<void> => {
  // create the server directory
  await promises.mkdir(`${cwd}/src/server`).catch((err) => {
    throw err;
  });
};

const createStyles = async (cwd: string): Promise<void> => {
  // create the styles directory
  await promises.mkdir(`${cwd}/src/styles`).catch((err) => {
    throw err;
  });

  // create the globals.scss file
  const globalsScss = `@tailwind base;
@tailwind components;
@tailwind utilities;';`;

  await promises
    .writeFile(`${cwd}/src/styles/globals.scss`, globalsScss)
    .catch((err) => {
      throw err;
    });
};

const createTypes = async (cwd: string): Promise<void> => {
  // create the types directory
  await promises.mkdir(`${cwd}/src/types`).catch((err) => {
    throw err;
  });
};

const createUi = async (cwd: string): Promise<void> => {
  // create the ui directory
  await promises.mkdir(`${cwd}/src/ui`).catch((err) => {
    throw err;
  });

  const brandingTSX = `interface Props {
  width?: string;
  height?: string;
  className?: string;
}
...
`;
  ...
  await promises
    .writeFile(`${cwd}/src/ui/branding.tsx`, brandingTSX)
    .catch((err) => {
      throw err;
    });
};

const createUtils = async (cwd: string): Promise<void> => {
  // create the utils directory
  await promises.mkdir(`${cwd}/src/utils`).catch((err) => {
    throw err;
  });
};

export const handleSrcScaffold = async (
  cwd: string,
  // answers: tScaffoldAnswers,
): Promise<void> => {
  await createSrc(cwd).catch((err) => {
    throw err;
  });
  await createApp(`${cwd}`).catch((err) => {
    throw err;
  });
  await createComponents(`${cwd}`).catch((err) => {
    throw err;
  });
  await createHooks(`${cwd}`).catch((err) => {
    throw err;
  });
  await createLayouts(`${cwd}`).catch((err) => {
    throw err;
  });
  await createServer(`${cwd}`).catch((err) => {
    throw err;
  });
  await createStyles(`${cwd}`).catch((err) => {
    throw err;
  });
  await createTypes(`${cwd}`).catch((err) => {
    throw err;
  });
  await createUi(`${cwd}`).catch((err) => {
    throw err;
  });
  await createUtils(`${cwd}`).catch((err) => {
    throw err;
  });
};
